...
However, as an advanced use case, you do have the option to distribute packages via Packagist (or any other directory). For that, the "Monorepo Split" feature must be enabled (see section [Monorepo Split](../README.md#monorepo-split) in the README), and then you can link to the package on its own repo.
...
See section [Synchronizing files from the upstream Gato GraphQL repo](../README.md#synchronizing-files-from-the-upstream-gato-graphql-repo) in the README to learn more.
